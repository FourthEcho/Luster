...
vec2 get_directional_lightmaps(vec3 position_scene, vec3 normal) {
    ...
    if (length_squared(lightmap_gradient) > 1e-12) {
        float bias = 1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY * 0.5;
        lightmap_mul.x = clamp01(dot(normalize(lightmap_dir), normal) + bias)
            * (DIRECTIONAL_LIGHTMAPS_INTENSITY + (1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY) / (clamp01(light_levels.x) + eps));
    }
    ...
    if (length_squared(lightmap_gradient) > 1e-12) {
        float bias = 1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY * 0.5;
        lightmap_mul.y = clamp01(dot(normalize(lightmap_dir), normal) + bias)
            * (DIRECTIONAL_LIGHTMAPS_INTENSITY + (1.0 - DIRECTIONAL_LIGHTMAPS_INTENSITY) / (clamp01(light_levels.y) + eps));
    }

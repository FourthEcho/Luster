...
#if defined INDIRECT_LIGHTING && !defined VOXY
layout(location = 2) out vec4 indirect_history_out;
/* RENDERTARGETS: 0,5,16 */
#else
/* RENDERTARGETS: 0,5 */
#endif
...
uniform sampler2D colortex1; // second-bounce irradiance generated by c3
uniform sampler2D colortex3; // receiver albedo written by c2
uniform sampler2D colortex17; // full-resolution 2x2 depth-slice irradiance cache (needed by shared GI helpers)
#ifndef VOXY
uniform sampler2D colortex16; // dedicated previous-frame GI history
#endif
#endif
...
void main() {
...
#ifdef INDIRECT_LIGHTING
    vec3 indirect_current = vec3(0.0);
#endif
...
#ifdef INDIRECT_LIGHTING
    vec3 indirect_current_raw = texture(colortex1, uv * taau_render_scale).rgb;
#if INDIRECT_BOUNCE_COUNT >= 3
    if (closest.z >= hand_depth && closest.z < 0.99999) {
        vec3 gi_receiver_pos = screen_to_view_space(closest, false, is_lod);
        vec3 gi_receiver_normal = indirect_reconstruct_normal(uv);
        vec3 gi_receiver_albedo = max(texelFetch(colortex3, texel, 0).rgb, vec3(0.02));
        float gi_weight = 0.0;
        vec3 bounce3 = indirect_gather_bounce3(
            uv, gi_receiver_pos, gi_receiver_normal, gi_receiver_albedo, gi_weight
        );
        indirect_current_raw += bounce3 * INDIRECT_THIRD_BOUNCE_ENERGY
            * clamp01(gi_weight + 0.25);
    }
#endif
    indirect_current_raw = max(indirect_current_raw, vec3(0.0)) * INDIRECT_INTENSITY;
#ifdef INDIRECT_TEMPORAL_ACCUMULATION
#ifndef VOXY
    vec3 previous_gi = texture(colortex16, previous_uv).rgb;
    float gi_history_alpha = clamp(1.0 - indirect_history_strength * alpha, 0.0, 1.0);
    indirect_current = mix(previous_gi, indirect_current_raw, gi_history_alpha);
#else
    indirect_current = indirect_current_raw;
#endif
#else
    indirect_current = indirect_current_raw;
#endif
...
#ifdef INDIRECT_LIGHTING
    current_color += indirect_current;
#endif
    ...
    // Interpolate AABB bounds across pixels. When GI owns colortex1/2, rebuild
    // the bounds from the current color and then widen them around the
    // indirect contribution so the temporal clipper does not reject GI.
    vec3 min_color;
    vec3 max_color;
    float flicker_reduction;

#ifdef INDIRECT_LIGHTING
    const vec2 taa_neighbors[9] = vec2[9](
        vec2(-1.0,  1.0), vec2(0.0,  1.0), vec2(1.0,  1.0),
        vec2(-1.0,  0.0), vec2(0.0,  0.0), vec2(1.0,  0.0),
        vec2(-1.0, -1.0), vec2(0.0, -1.0), vec2(1.0, -1.0)
    );
    vec3 gi_min = vec3(1e20);
    vec3 gi_max = vec3(-1e20);
    for (int i = 0; i < 9; ++i) {
        vec2 sample_uv = clamp01(pos + taa_neighbors[i] * view_pixel_size * taau_render_scale);
        vec3 neighbor = reinhard(texture(colortex0, sample_uv).rgb);
        neighbor = rgb_to_ycocg(neighbor);
        gi_min = min(gi_min, neighbor);
        gi_max = max(gi_max, neighbor);
    }
    vec3 gi_expansion = abs(rgb_to_ycocg(reinhard(indirect_current)));
    min_color = gi_min - gi_expansion * 0.75;
    max_color = gi_max + gi_expansion * 1.25;
#else
    min_color = texture(colortex1, pos).rgb * 2.0 - 1.0;
    max_color = texture(colortex2, pos).rgb * 2.0 - 1.0;
#endif
...
#else // TAA disabled
#ifdef INDIRECT_LIGHTING
    indirect_current = texture(colortex1, uv * taau_render_scale).rgb * INDIRECT_INTENSITY;
    result = vec4(texelFetch(colortex0, texel, 0).rgb + indirect_current, 1.0);
#else
...
#if defined INDIRECT_LIGHTING && !defined VOXY
    indirect_history_out = vec4(indirect_current, 1.0);
#endif

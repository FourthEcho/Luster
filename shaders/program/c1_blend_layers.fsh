...
layout(location = 1) out vec3 irradiance_cache;
...
vec2 cache_atlas_local_uv(vec2 frag) {
    vec2 tile_res = view_res * 0.5;
    return fract(frag / max(tile_res, vec2(1.0)));
}

void write_irradiance_cache() {
    vec2 tile_res = view_res * 0.5;
    ...
    irradiance_cache = weight_sum > 1e-4 ? sum / weight_sum : vec3(0.0);
}

void main() {
    ivec2 texel = ivec2(gl_FragCoord.xy);
    write_irradiance_cache();

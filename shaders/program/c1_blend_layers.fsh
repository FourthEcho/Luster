...
uniform sampler2D colortex1; // gbuffer data 0 (albedo, normal, lightmap)
uniform sampler2D colortex18; // filtered quarter-res indirect lighting
#endif
...
#ifdef INDIRECT_LIGHTING
    // GI is generated by deferred5-12, so it must be consumed here, after
    // the final temporal/filter pass. Deferred4 runs before that chain and
    // cannot see the current frame's finished GI. Add GI before translucent
    // layers and atmospheric fog so the indirect contribution is transported
    // through the same later composition as the directly lit scene.
    ivec2 gi_albedo_texel = ivec2(
        uv * view_res * taau_render_scale
    );
    vec4 gi_gbuffer = texelFetch(colortex1, gi_albedo_texel, 0);
    mat4x2 gi_data = mat4x2(
        unpack_unorm_2x8(gi_gbuffer.x),
        unpack_unorm_2x8(gi_gbuffer.y),
        unpack_unorm_2x8(gi_gbuffer.z),
        unpack_unorm_2x8(gi_gbuffer.w)
    );
    vec3 gi_albedo = vec3(gi_data[0], gi_data[1].x);

    // AO is already part of the deferred lighting pipeline. Recover its
    // existing ambient-occlusion value from the same shading inputs used by
    // d4 rather than inventing a second AO term here. GI itself was already
    // visibility-tested during the bounce pass, so use neutral AO for the
    // final upsample to avoid multiplying AO twice.
    float gi_depth = texture(depthtex0, refracted_uv).x;
    if (gi_depth < 1.0 && gi_depth >= hand_depth) {

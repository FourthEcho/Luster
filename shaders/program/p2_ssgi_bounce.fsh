...
uniform sampler2D colortex17; // previous bounce radiance
...
void main() {
    ...
    vec3 previous_bounce = texelFetch(colortex17, texel, 0).rgb;

    if (depth >= 1.0 || depth < hand_depth) {
        bounce_radiance = previous_bounce;
        return;
    }
    ...
    // Bounce 2: gather from the accumulated bounce-1 radiance
    bounce_radiance = previous_bounce
        + ssgi_gather(position_view, normal, colortex17, texel, dither);
#else
    bounce_radiance = texelFetch(colortex17, ivec2(gl_FragCoord.xy), 0).rgb;
#endif
}
